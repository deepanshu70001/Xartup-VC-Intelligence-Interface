      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-6 sm:top-1/2 -translate-x-1/2 sm:-translate-y-1/2 w-[calc(100vw-2rem)] max-w-md max-h-[calc(100vh-3rem)] sm:max-h-[85vh] overflow-y-auto bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl border border-neutral-200 dark:border-neutral-800 z-50 p-5 sm:p-6"
          >
                ...
                <div className="flex flex-wrap gap-2">
                  {['All', 'DevTools', 'Productivity', 'Fintech', 'Design', 'AI/ML', 'Biotech', 'Cybersecurity', 'Healthcare', 'Robotics'].map(ind => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-sm rounded-full border transition-colors ${(filters.industry === ind || (!filters.industry && ind === 'All'))
                ...
                <div className="flex flex-wrap gap-2">
                  {stages.map(stage => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-sm rounded-full border transition-colors ${filters.stages?.includes(stage)
                          ? 'bg-indigo-100 dark:bg-indigo-900/30 border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300'
                          : 'bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 text-neutral-600 dark:text-neutral-400 hover:border-neutral-300 dark:hover:border-neutral-600'
                        }`}
                    >
                ...
                <div className="flex flex-wrap gap-2">
                  {employeeRanges.map(range => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-sm rounded-full border transition-colors ${filters.employeeRanges?.includes(range)
                          ? 'bg-indigo-100 dark:bg-indigo-900/30 border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300'
                          : 'bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 text-neutral-600 dark:text-neutral-400 hover:border-neutral-300 dark:hover:border-neutral-600'
                        }`}
                    >
